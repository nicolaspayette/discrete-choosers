...
/**
 * the least squares filter as implemented in : http://www.cs.tut
 * .fi/~tabus/course/ASP/LectureNew10.pdf
...
public class RecursiveLeastSquaresRegression extends WeightedNumericalRegression {
    ...
    public RecursiveLeastSquaresRegression(
        double[][] uncertainty,
        double[] beta,
        ...
        for (int row = 0; row < dimension; row++) {
            for (int column = 0; column < dimension; column++) {
                ...
                assert (Double.isFinite(prime[row][column])) :
                    "pi " + pi[column] + " , kalman: " + kalman[column];
    ...
    /**
     * if sigma^2 is infinite the kalman will be 0 which means that the only thing actually changing
     * is P increasing. This method just applies that part
     */
