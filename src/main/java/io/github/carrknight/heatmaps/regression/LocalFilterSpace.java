...
/**
 * basically we build a 1D filter for each possible option but given a similarity function we can
 * update the prediction for one object given the observed reward for another
 */
public class LocalFilterSpace<O, R, C> implements BeliefState<O, R, C> {


    private final Map<O, OneDimensionalFilter> filters = new HashMap<>();
    ...
    public void observe(Observation<O, R, C> observation) {
        ...
        double reward = utility.extractUtility(
