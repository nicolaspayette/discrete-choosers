...
/**
 * you have a prior on what a "good" reward distribution looks like and what a "bad" reward
 * distribution looks like. This filter tries to guess if what we keep filter is a good or a bad
 * distribution. Good and bad priors are normal distributions
 */
public class GoodBadFilter implements OneDimensionalFilter {
    ...
    public double predict() {
        return probabilityBeingAGoodSpot * goodAverage
            + (1 - probabilityBeingAGoodSpot) * badAverage;
    ...
    public void observe(double evidence, double weight) {
        ...
        if (badPosterior + goodPosterior == 0) {
            ...
            if (evidence > goodAverage) {
                ...
                probabilityBeingAGoodSpot =
                    .5d; //if you are here that's some very poor averages/std you got
            }
        } else {
    ...
    public static Function<Double, Double> normalPDF(double mean, double standardDeviation) {
        return new Function<Double, Double>() {
            ...
            public Double apply(Double x) {
                return
                    Math.exp(-Math.pow(x - mean, 2) / (2 * standardDeviation * standardDeviation)) /
