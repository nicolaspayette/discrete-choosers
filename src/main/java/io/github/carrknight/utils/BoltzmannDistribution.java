...
/*
 * This class is copied from the BURLAP library which is covered by Apache license and it's
 * available here:
 * https://github.com/jmacglashan/burlap
 * burlap/src/main/java/burlap/datastructures/BoltzmannDistribution.java
 * the original license is available here:
 * https://raw.githubusercontent.com/jmacglashan/burlap/master/LICENSE
 *
 *
 * it is only very slightly modified so that it takes random doubles from outside rather than
 * having its own random generator
 */
...
/**
 * This class provides methods for computing, sampling, and working with Boltzmann
 * distributions/Softmax. The input is a set of "preference" values. The output is a probability
 * distribution over those preferences with elements that had higher preferences having higher
 * probability. Computation is performed using a log sum exponential re-expression trick that avoids
 * overflow errors.
 *
...
public class BoltzmannDistribution {
    ...
    /**
     * Initializes the distribution with the preference values that are to be turned into a soft max
    ...
    /**
     * Initializes the distribution with the preference values that are to be turned into a soft max
     * probability distribution and a temperature value to control how deterministic the probability
     * output is. Lower temperature values result in a more deterministic distribution; higher
     * values produce a more uniform distribution.
     *
     * @param preferences the preference values to turn into a probability distribution
    ...
    /**
     * Returns the output probability distribution. This method computes the probabilities lazily
     ...
     */
    public int sample(double randomDouble) {
    ...
    /**
     * Computes the probability distribution. This method uses the log sum exp trick to avoid
     * overflow issues.
     */
